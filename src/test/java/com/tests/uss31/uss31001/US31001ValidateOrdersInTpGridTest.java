...
public class US31001ValidateOrdersInTpGridTest extends BaseTest {
	...
	public void setUp() {
		...
		detailedProdListTp5 = MongoReader.grabProductDetailedModel("US31001PartyHostBuysForCustomerTpTest" + "TP5").get(0);
		...
		detailedProdListTp6 = MongoReader.grabProductDetailedModel("US31001PartyHostBuysForCustomerTpTest" + "TP6").get(0);
		...
		expectedModel5.setIsDiscontinued(detailedProdListTp5.getStockData().getIsDiscontinued().contentEquals("1") ? "Yes" : "No");
		...
		expectedModel5.setProductQty(StockCalculations.calculateStockToInt(detailedProdListTp5.getStockData().getQty(), productsListTp5.get(0).getQuantity()));
		...
		expectedModel6.setIsDiscontinued(detailedProdListTp6.getStockData().getIsDiscontinued().contentEquals("1") ? "Yes" : "No");
		...
		expectedModel6.setProductQty(StockCalculations.calculateStockToInt(detailedProdListTp6.getStockData().getQty(), productsListTp6.get(0).getQuantity()));
	...
	public void runCron() throws Exception {
        ...
        // we modify the fifth product to have:recomandion to postpone because the stock is not enough
		...
		stockModel.setEarliestAvailability(DateUtils.addDaysToAAGivenDate(DateUtils.getCurrentDate("yyyy-MM-dd"), "yyyy-MM-dd", -1));
