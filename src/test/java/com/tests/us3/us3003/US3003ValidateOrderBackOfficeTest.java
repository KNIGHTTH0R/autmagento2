...
public class US3003ValidateOrderBackOfficeTest extends BaseTest {
	...
	public void setUp() {
		...
		try {
			...
			input = new FileInputStream(Constants.RESOURCES_PATH + Constants.US_03_FOLDER + File.separator + "us3003.properties");
			prop.load(input);
		...
		List<OrderModel> orderModelList = MongoReader.getOrderModel("US3003Test" + Constants.GRAB);
		productsList = MongoReader.grabBasicProductModel("US3003Test" + Constants.GRAB);
		shippingModelList = MongoReader.grabShippingModel("US3003Test" + Constants.CALC);
		calcDetailsModelList = MongoReader.grabCalcDetailsModels("US3003Test" + Constants.CALC);
