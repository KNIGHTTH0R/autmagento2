...
public class HttpSoapConnector {
	...
	public static SOAPMessage soapCreateProduct(ProductDetailedModel product) throws SOAPException, IOException {
		...
		SOAPMessage soapResponse = soapConnection.call(createProduct(product, sessID), MongoReader.getBaseURL() + UrlConstants.API_URI);
	...
	protected static String performLogin() throws SOAPException, IOException {
		SOAPConnectionFactory soapConnectionFactory = SOAPConnectionFactory.newInstance();
		SOAPConnection soapConnection = soapConnectionFactory.createConnection();
		SOAPMessage soapResponse = soapConnection.call(createLoginRequest(Credentials.LOGIN_USER_SOAP, Credentials.LOGIN_PASS_SOAP), MongoReader.getBaseURL() + UrlConstants.API_URI);
		String result = "";
