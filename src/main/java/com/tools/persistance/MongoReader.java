...
public class MongoReader extends MongoConnector {
	...
	public static List<CustomerFormModel> grabCustomerFormModels(String testName) {
		...
		workingDB = mongoClient.getDB(testName);
		DBCursor cursor = workingDB
				.getCollection(MongoConstants.CUSTOMER_MODEL).find();
	...
	public static List<CustomerConfigurationModel> grabCustomerConfigurationModels(String testName) {
		...
		workingDB = mongoClient.getDB(testName);
		DBCursor cursor = workingDB
				.getCollection(MongoConstants.CUSTOMER_CONFIGURATION_MODEL).find();
		...
		try {
			while (cursor.hasNext()) {
				...
				result.setEmailActive(MongoUtils.checkField(dbObject,
						MongoConstants.EMAIL_ACTIVE));
				result.setAccountActive(MongoUtils.checkField(dbObject,
						MongoConstants.ACCOUNT_ACTIVE));
