...
public class ImportOrdersSteps {
	...
	public void validateOrders(List<DBOrderModel> shopOrderList, List<NavOrderModel> navOrderList) throws Exception {
		...
		// Assert.assertTrue("Failure: The list size are not equal",
		...
		for (DBOrderModel order : shopOrderList) {
			...
			if (compare.getIncrementId() == null) {
				CustomVerification.verifyTrue("Failure: Could not validate order " + order.getIncrementId(), compare == null);
			} else {
				...
				// importOrders.validateCartType(order.getCartType(),
				// compare.getShopCartType());
